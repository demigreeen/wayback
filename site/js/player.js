...
const WBPlayer = (() => {
  ...
  // Зум камеры, пока она следит за тренировками. Он же — предельное
  // приближение: ближе камера не подходит нигде, даже в общем плане.
  // Был 13 — город рассматривался по кварталам, и камера переезжала
  // за каждой пробежкой. 11 — вчетверо дальше: город целиком, и камера
  // сдвигается только при смене места.
  ...
  //
  // Радиус растёт вместе с отдалением камеры: при зуме 13 было 4 км,
  // при 11 — вчетверо больше. Так точка отпускает камеру, уходя на ту же
  // долю кадра, что и раньше, а в пределах города камера не двигается.
  //
  // Оговорка про превью. Зона ограничена ещё и кадром, а кадр превью меньше
  // видео: вертикальное превью на ноутбуке — около 530 px, половина зоны
  // покоя в нём при зуме 11 — ~13 км, а не 16. Поэтому в вертикали превью
  // камера переезжает чуть раньше, чем в скачанном видео. В горизонтали
  // и на видео решает радиус.
  const HOLD_RADIUS_M = 16000;
  ...
  function makeArc(from, to) {
    ...
    // Делитель подобран под FOLLOW_ZOOM: 12000 при зуме 11 даёт в верхней
    // точке перелёта тот же зум, что 3000 при прежнем 13
